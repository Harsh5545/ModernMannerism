 ...
 handleMouseEnter} : undefined} // Open on hover for desktop
                                    onMouseLeave={!isMobile ? () => {setOpenDropdown(false)
handleMouseLeave} : undefined} // Close on mouse leave for desktop
                                >
                                    {link.title}
                                    <svg
                                    ...
                                    <div className={`absolute mt-2  bg-white rounded-lg shadow-lg`}>
                                        {link.subLinks.map((subLink, j) => (
                                            <div
                                                ...
                                                onMouseEnter={!isMobile ? () => {setHoveredSubLink(subLink.title) 
setOpenDropdown(true)} : undefined}
                                                onMouseLeave={!isMobile ? () => {setHoveredSubLink(null) 
setOpenDropdown(false)}: undefined}
                                            >
                                                <button
                                                ...
                                                {hoveredSubLink === subLink.title && subLink.subLinks && (
                                                    <div className={`absolute text-left left-60 text-sm font-semibold mt-0 top-0 max-w-full bg-white shadow-lg rounded-lg ${isMobile ? "w-full" : "w-60"}`}>
